...
void Runner(TString what = "plot")
{
    ...
    {
        throw std::runtime_error("Runner : cannot set inital beam energy for given pressure ");
    }
    Print(beam, target, light, ebeam_i, pressure, what.Data());

    auto args {TString::Format("(\"%s\", \"%s\", \"%s\")", beam.c_str(), target.c_str(), light.c_str())};
    auto extargs {TString::Format("(\"%s\", \"%s\", \"%s\", %f, %d)", beam.c_str(), target.c_str(), light.c_str(),
                                  ebeam_i, pressure)};
